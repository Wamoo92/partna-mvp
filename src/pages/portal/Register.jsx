...
export default function Register() {
  ...
  async function handleStep3() {
    ...
    try {
      ...
      // ── 1. Call Edge Function — creates auth user and updates customer ──
      // Uses service role server-side so no RLS issues
      const res  = await fetch(`${SUPABASE_URL}/functions/v1/complete-customer-registration`, {
        method:  'POST',
        headers: {
      ...
      // ── 3. Navigate to campaign selection ─────────────────────────────
              ...
              <>
                <div style={{ display: 'flex', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input style={input} type="text" placeholder="John" value={firstName} onChange={e => setFirstName(e.target.value)} onFocus={focusInput} onBlur={blurInput} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={label}>Last name <span style={{ color: '#CC3939' }}>*</span></label>
                    <input style={input} type="text" placeholder="Doe" value={lastName} onChange={e => setLastName(e.target.value)} onFocus={focusInput} onBlur={blurInput} />
                  </div>
                </div>
                <div>
                  <label style={label}>Other names <span style={{ fontWeight: 500, color: '#898B90' }}>(optional)</span></label>
                  <input style={input} type="text" placeholder="Middle name" value={otherNames} onChange={e => setOtherNames(e.target.value)} onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={label}>Phone number <span style={{ color: '#CC3939' }}>*</span></label>
                  <input style={input} type="tel" placeholder="+256 7XX XXX XXX" value={phone} onChange={e => setPhone(e.target.value)} onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={label}>Email address <span style={{ color: '#CC3939' }}>*</span></label>
                  <input style={input} type="email" placeholder="john@email.com" value={email} onChange={e => setEmail(e.target.value)} onFocus={focusInput} onBlur={blurInput} />
                  <p style={{ fontSize: 12, fontWeight: 500, color: '#898B90', margin: '4px 0 0' }}>Used for account recovery only.</p>
                </div>
                <button style={btnPrimary} onClick={handleStep1} disabled={loading}>
                  {loading ? <><div className="spinner spinner-sm spinner-light" /> Sending code…</> : 'Continue'}
                </button>
              </>
            )}
            ...
            {step === 2 && (
              <>
                ...
                <div>
                  ...
                  <input style={{ ...input, textAlign: 'center', letterSpacing: '0.4em', fontSize: 28, fontWeight: 600 }} type="text" inputMode="numeric" maxLength={5} placeholder="·····" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, '').slice(0, 5))} onFocus={focusInput} onBlur={blurInput} />
                </div>
                <button style={btnPrimary} onClick={handleStep2} disabled={loading}>
                  {loading ? <><div className="spinner spinner-sm spinner-light" /> Verifying…</> : 'Verify code'}
                </button>
                <p style={{ textAlign: 'center', fontSize: 14, fontWeight: 500, color: '#959687', margin: 0 }}>
                  Didn't receive it?&nbsp;<button onClick={() => { setStep(1); setOtp('') }} style={linkBtn}>Go back</button>
                </p>
              </>
            )}
            ...
            {step === 3 && (
              <>
                ...
                <div>
                  ...
                  <input style={{ ...input, textAlign: 'center', letterSpacing: '0.5em', fontSize: 28, fontWeight: 600 }} type="password" inputMode="numeric" maxLength={4} placeholder="····" value={pin} onChange={e => setPin(e.target.value.replace(/\D/g, '').slice(0, 4))} onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={{ ...label, textAlign: 'center' }}>Confirm PIN</label>
                  <input style={{ ...input, textAlign: 'center', letterSpacing: '0.5em', fontSize: 28, fontWeight: 600 }} type="password" inputMode="numeric" maxLength={4} placeholder="····" value={confirmPin} onChange={e => setConfirmPin(e.target.value.replace(/\D/g, '').slice(0, 4))} onFocus={focusInput} onBlur={blurInput} />
                </div>
                ...
                <button style={btnPrimary} onClick={handleStep3} disabled={loading}>
                  {loading ? <><div className="spinner spinner-sm spinner-light" /> Creating account…</> : 'Create account'}
                </button>
              </>
            )}
